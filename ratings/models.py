...
class MoviesIndexPage(RoutablePageMixin, Page):
    ...
    def top_recommended(self, request):
        ...
        """
        filtered_results = MoviePage.objects.filter(rating__gte=7).order_by('title')
    ...
    def recommended(self, request):
        ...
        View function for must-watch movies
        """
        filtered_results = MoviePage.objects.filter(rating__gte=4).filter(rating__lte=6).order_by('title')
        movie_count = self.get_movie_count(filtered_results)
    ...
    def not_recommended(self, request):
        ...
        View function for must-watch movies
        """
        filtered_results = MoviePage.objects.filter(rating__lte=3).order_by('title')
        movie_count = self.get_movie_count(filtered_results)
...
class TvIndexPage(RoutablePageMixin, Page):
    ...
    def top_recommended(self, request):
        ...
        View function for must-watch movies
        """
        filtered_results = TvPage.objects.filter(rating__gte=7).order_by('title')
    ...
    def recommended(self, request):
        ...
        View function for must-watch movies
        """
        filtered_results = TvPage.objects.filter(rating__gte=4).filter(rating__lte=6).order_by('title')
        tv_count = self.get_tv_count(filtered_results)
    ...
    def not_recommended(self, request):
        ...
        View function for must-watch movies
        """
        filtered_results = TvPage.objects.filter(rating__lte=3).order_by('title')
        tv_count = self.get_tv_count(filtered_results)
